...
function all($link)
{
    ...
    $columns     = array('typpub', 'typpub_name');

    // Búsqueda
    $searchValue = isset($_REQUEST['search']['value']) ? $_REQUEST['search']['value'] : '';

    $condicion   = ($status == $todos) ? "typpub_status IN (?, ?)" : "typpub_status = ?";
    $condicion2  = ($status == $todos) ? "typpub_status IN ($activo, $inactivo)" : "typpub_status = $status";

    // Consulta SQL con una consulta preparada para seleccionar datos
    $sql = "SELECT * FROM cat_type_publication WHERE $condicion";
    ...
    if ($stmt->execute()) {
        ...
        if ($result->num_rows > 0) {
            ...
            $data = array(
                ...
                'recordsFiltered' => mysqli_num_rows(mysqli_query($link, "SELECT * FROM cat_type_publication WHERE $condicion2")), // Total de registros después del filtrado
                'status'          => true,
                'message'         => ''
            );
        } else {
