...
function all($link)
{
    ...
    $columns     = array('cat_id', 'cat_name', 'cat_description');

    // Búsqueda
    $searchValue = isset($_REQUEST['search']['value']) ? $_REQUEST['search']['value'] : '';

    $condicion   = ($status == $todos) ? "cat_status IN (?, ?)" : "cat_status = ?";
    $condicion2  = ($status == $todos) ? "cat_status IN ($activo, $inactivo)" : "cat_status = $status";

    // Consulta SQL con una consulta preparada para seleccionar datos
    $sql = "SELECT * FROM cat_category WHERE $condicion";
    ...
    if ($stmt->execute()) {
        ...
        if ($result->num_rows > 0) {
            ...
            $data = array(
                ...
                'recordsFiltered' => mysqli_num_rows(mysqli_query($link, "SELECT * FROM cat_category WHERE $condicion2")), // Total de registros después del filtrado
                'status' => true,
                'message' => ''
            );
        } else {
...
function delete($link)
{
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        try {
            ...
            $query1 = "DELETE FROM cat_category WHERE cat_id = ?";
            $stmt1 = $link->prepare($query1);
            $stmt1->bind_param("i", $cat_id);
            $stmt1->execute();
